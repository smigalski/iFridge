...
from ui_Usermanagement import get_user_management_instance

ImportierteInstanz = get_user_management_instance()


#Debug
#users = ImportierteInstanz.get_all_users()

try:
    users = ImportierteInstanz.get_all_users()
    print("Aktuelle Nutzerliste:", users)
except Exception as e:
    print(f"Fehler beim Abrufen der Nutzerliste: {e}")
...
class Ui_Produktauswahl(object):
    ...
    def setupUi(self, Produktauswahl):
        ...
        # Initialisieren der Mitarbeiternamen in die ComboBox
        self.update_users()
        ...
        #Debug
        self.pushButton.clicked.connect(self.print_users)
    ...
    # diese Methode ruft die Usernamen aus der ui_Usermanagement.py ab
    def update_users(self):
        users = ImportierteInstanz.get_all_users()
        self.comboBox.clear()
        self.comboBox.addItems(users.keys())

    # diese Methode updatet die Anzeige (Label 4) für die Guthaben der Mitarbeiter
    def update_balance(self):
        selected_user = self.comboBox.currentText() # es wird immer das Guthaben des aktuell ausgewählten Mitarbeiters angezeigt
        if selected_user:
            balance = ImportierteInstanz.get_all_users()[selected_user]['balance']
            self.label_4.setText(f"Guthaben: {balance} €")
        else:
            self.label_4.setText("Guthaben: - €")
